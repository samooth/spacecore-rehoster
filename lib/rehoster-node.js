...
export default class RehosterNode {
  ...
  async setup () {
    ...
    if (this.isTornDown) throw new Error('Torn down')
  ...
  async _setupChildCores (bee) {
    ...
    const appendListener = debounceify(
      async () => {
        ...
        // TODO: possible race condition (investigate):
        // if this node is torn down between isTornDown-check and here, it shouldn't add children
        // (ugly solution would be to detect this here and call the teardown logic on all newChildren)
