...
class RehosterNode extends ReadyResource {
  ...
  async _setupChildLogic () {
    ...
    if ((await isRehoster(bee))) {
      ...
      this._watcher = bee.watch(
        getMetadataSubRange(),
        { map: snapshot => snapshot.sub(METADATA_SUB) }
      )
...
function getMetadataSubRange () {
  const sub = b4a.from(METADATA_SUB)
  return {
    gte: b4a.concat([sub, b4a.from('\x00')]),
    lt: b4a.concat([sub, b4a.from('\x01')])
  }
}
